...
exports.signup = (req, res) => {
  ...
  user.save((err, user) => {
    ...
    if (req.body.roles) {
      ...
      Role.findOne({ name: "icv_user" }, (err, role) => {
        if (err) {
          res.status(500).send({ message: err });
          return;
        }
...
exports.addModerator = (req, res) => {
  ...
  user.save((err, user) => {
    ...
    if (req.body.roles) {
      ...
      Role.findOne({ name: "icv_moderator" }, (err, role) => {
        if (err) {
          res.status(500).send({ message: err });
          return;
        }
        ...
        user.save(err => {
          ...
          res.status(200).send({ message: "Moderator was registered successfully!" });
        });
      });
    }
  });
...
exports.addAdmin = (req, res) => {
  ...
  user.save((err, user) => {
    ...
    if (req.body.roles) {
      ...
      Role.findOne({ name: "icv_admin" }, (err, role) => {
        if (err) {
          res.status(500).send({ message: err });
          return;
        }
